/**
 * User Model
 * ----------
 * Stores account credentials and role info for both admin users and riders.
 *
 * Features:
 *  - Automatic password hashing via a pre-save hook (bcryptjs, 12 rounds)
 *  - Instance method `matchPassword` for login verification
 *  - Email stored in lowercase to avoid case-sensitivity issues
 */

import mongoose from 'mongoose';
import bcrypt from 'bcryptjs';

const userSchema = new mongoose.Schema({
  name: {
    type: String,
    required: [true, 'Name is required'],
    trim: true,
    maxlength: [100, 'Name cannot exceed 100 characters'],
  },

  email: {
    type: String,
    required: [true, 'Email is required'],
    unique: true,
    lowercase: true,
    trim: true,
    match: [
      /^[^\s@]+@[^\s@]+\.[^\s@]+$/,
      'Please provide a valid email address',
    ],
  },

  password: {
    type: String,
    required: [true, 'Password is required'],
    minlength: [6, 'Password must be at least 6 characters'],
    select: false, // excluded by default from query results
  },

  role: {
    type: String,
    enum: {
      values: ['admin', 'rider'],
      message: 'Role must be either admin or rider',
    },
    default: 'admin',
  },

  createdAt: {
    type: Date,
    default: Date.now,
  },
});

// ── Pre-save hook: hash password before persisting ──────────────────────
userSchema.pre('save', async function (next) {
  // Only hash if the password field was modified (or is new)
  if (!this.isModified('password')) return next();

  try {
    const salt = await bcrypt.genSalt(12);
    this.password = await bcrypt.hash(this.password, salt);
    next();
  } catch (error) {
    next(error);
  }
});

// ── Instance method: compare entered password with stored hash ──────────
/**
 * Compare a plain-text password against the stored bcrypt hash.
 * @param  {string}  enteredPassword - The password the user typed in
 * @returns {Promise<boolean>}       true if they match
 */
userSchema.methods.matchPassword = async function (enteredPassword) {
  return bcrypt.compare(enteredPassword, this.password);
};

const User = mongoose.model('User', userSchema);

export default User;
